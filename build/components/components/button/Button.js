...
const Button = ({ loading, disabled, title, className, size = 'lg', type = 'button', variant = 'primary', prefixIcon = null, suffixIcon = null, ...rest }) => {
    return (_jsxs("button", { ...rest, type: type, disabled: disabled || loading, className: `bg-[#3699FF] text-base py-2 md:py-2 px-2 md:px-4 rounded-[4px] disabled:bg-grey border disabled:text-gray-50 whitespace-nowrap ${variants[variant]} ${sizes[size]} ${className} ${loading && 'py-3 md:py-4 px-8 md:px-12 bg-[#EEF5FC] text-[#ABBED1]'}`, children: [prefixIcon && (_jsxs(_Fragment, { children: [" ", prefixIcon, "\u00A0\u00A0\u00A0"] })), loading ? _jsx(Spinner, {}) : _jsx(_Fragment, { children: title }), suffixIcon && (_jsxs(_Fragment, { children: [" \u00A0\u00A0\u00A0", suffixIcon] }))] }));
};
