      ...
      <div className='px-5 pt-5 flex flex-wrap items-center justify-between mx-auto max-w-[1350px]'>
        ...
        <div className=' items-center flex gap-1'>
          <div className='hidelogin lg:flex gap-4 text-base font-bold'>
            ...
            <button onClick={() => navigate('/register')} className="bg-[#065373] p-3 px-12 rounded-xl text-white">Create Web Account</button>
          </div>
        ...
        <ClickOutside onclickoutside={() => setNav(true)} className={(!nav && (width <= 1024)) ? 'fixed left-0 top-0 w-full border-r border-r-gray-300 bg-[#fff] ease-in-out duration-500 shadow-lg z-50' : 'ease-in-out duration-500 fixed top-[-100%]'}>
          <div className='flex flex-col justify-center items-center w-full p-6'>
            ...
            <button onClick={() => navigate('/login')} className="p-5 px-12 text-black mb-5 rounded-2xl border-2 border-black w-full">Login</button>
            <button onClick={() => navigate('/register')} className="bg-black p-5 px-12 rounded-2xl text-white mb-5 w-full">Create Webss Account</button>
